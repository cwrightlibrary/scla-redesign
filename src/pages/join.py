...
with st.expander("Individual Memberships"):
    ...
    st.markdown("**Library school students**, please see below information about our joint membership opportunities.")
    ...
    individual_membership_df = pd.DataFrame(individual_membership_table, columns=["Membership Type", "Price"])
    ...
    individual_salary_df = pd.DataFrame(individual_salary_table, columns=["Salary", "Price"])
...
with st.expander("Institutional Memberships"):
    ...
    st.write("Institutional membership rates are calculated on the library's total annual budget.")
    ...
    st.info(":material/info: To request an Institutional Membership for your library, please contact the Membership Chair at [secondvpscla@gmail.com](mailto:secondvpscla@gmail.com)")
...
with st.expander("Joint Memberships"):
    ...
    st.info(":material/info: [Joint membership forms can be submitted online through ALA.](https://www.ala.org/cfapps/jntapp/index.cfm?urlcode=ST-SC)\n\nFor more information on submitting membership forms by mail, see the **Join Offline** section of ALA's [Joint Membership Program site](http://www.ala.org/groups/joint-membership-program).")

st.divider()

st.info(":material/info: Questions? Contact [Courtney Waldrup](mailto:cwaldrup@pmpamc.com) (SCLA Executive Director) to learn more.")